...
static TxStruct ti;  // Used to compute Decred hashPrefix in Phase 1, Decred
                     // witness hash in Phase 2 or legacy digest in Phase 2.
...
foreach I (idx1):
    ...
    if (I has orig_hash)
        Request orig_hash, orig_index                         STAGE_REQUEST_1_ORIG_INPUT
    if (Decred)
        Return I
...
foreach O (idx1):
    ...
    if (O has orig_hash)
        Request orig_hash, orig_index                         STAGE_REQUEST_2_ORIG_OUTPUT
    if (Decred)
        Return O
    Display output
...
static bool signing_check_output(TxOutputType *txoutput) {
  ...
  // Skip output confirmation in replacement transactions.
  bool needs_confirm = !is_replacement;

  // Don't allow adding new OP_RETURN outputs in replacement transactions.
  if (txoutput->script_type == OutputScriptType_PAYTOOPRETURN &&
      is_replacement && !txoutput->has_orig_hash) {
    fsm_sendFailure(FailureType_Failure_ProcessError,
                    _("Adding new OP_RETURN outputs in replacement "
  ...
  if (is_change_output(&info, txoutput)) {
    // Skip confirmation of change-outputs.
    needs_confirm = false;
  ...
  int co = compile_output(coin, &root, txoutput, &bin_output, needs_confirm);
  if (needs_confirm) {
    layoutProgress(_("Signing transaction"), progress);
  }
  if (co < 0) {
...
static bool signing_check_orig_input(TxInputType *orig_input) {
  ...
  if (!have_orig_verif_input && orig_input->address_n_count != 0 &&
    ...
    // Derive node.public_key and fill script_sig with scriptPubKey.
...
static bool signing_check_orig_output(TxOutputType *orig_output) {
  ...
  // Add output to original legacy digest computation (ti).
  // Add output to original TXID computation (tp).
  ...
  if (idx2 != output.orig_index) {
    ...
    // Check original output corresponding to the current output.
    ...
    // If current output is a change-output, then so must the original one be.
    ...
    if (!is_change) {
      if (output.amount < orig_output->amount) {
        fsm_sendFailure(
            ...
            _("Reducing original output amounts is not supported."));
        signing_abort();
        return false;
      }
    }
...
static bool signing_confirm_tx(void) {
  ...
  // TODO What should we do if total_in < change_out?
  ...
  if (is_replacement) {
    ...
    // TODO What should we do if orig_total_in < orig_change_out?
    uint64_t orig_spending = orig_total_in - orig_change_out;
    uint64_t orig_fee = 0;
    if (orig_total_out <= orig_total_in) {
      orig_fee = orig_total_in - orig_total_out;
    }

    // Replacement transactions are only allowed to make amendments which do not
    // increase the amount that we are spending on external outputs. Additional
    // funds can only go towards the fee, which is confirmed by the user.
    ...
    // Final confirmation.
    // TODO show correct fee change even when one fee comes out negative.
...
static bool signing_check_orig_tx(void) {
  ...
  if (!have_orig_verif_input) {
    fsm_sendFailure(FailureType_Failure_ProcessError,
                    _("Each original transaction must specify address_n for at "
                      "least one input."));
    signing_abort();
    return false;
  }
